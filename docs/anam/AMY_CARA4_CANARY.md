...
`GET /api/anam/amy/readiness` is a content-free, bearer-authenticated readiness view that reuses the worker secret. It reports the effective session, recovery, Hermes, memory, and promotion gates without exposing secret or memory content. `POST /api/anam/hermes/worker` uses the same bearer authentication, accepts a maximum 32 KiB JSON body, and supports `claim`, `begin`, `ack`, `fail`, read-only `status`, and approval-gated `retire_stale` operations. Claims and backlog operations require the exact `amy_anam_hermes_worker_v2` protocol value before any lease or mutation. Backlog responses contain counts, timestamps, and a digest only; they never contain job/session identifiers or transcript/generated content. Stop the worker during a server/worker update, deploy the server, then restart the matching worker; mixed versions fail before claim rather than bypassing `begin`. See `AMY_HERMES_BACKLOG_OPERATIONS.md` for the inspect-before-retire checkpoint procedure.
