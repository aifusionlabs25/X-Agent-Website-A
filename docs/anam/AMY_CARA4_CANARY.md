...
Run `npm run hermes:amy-anam-cleanup` independently of the worker to unlink expired final files and stale temporary files. The command loads no Anam, Redis, Hermes, AgentMail, or email secret; it returns only a content-free count summary, does not create a missing output directory, and reports linked, future-dated, changed, or locked artifacts instead of following them. If a writer owns the shared lock, independent cleanup skips without touching artifacts and exits with code `3`, so a future scheduler must retry soon. A continuously running worker returns `local_output_busy` before claim and polls again; the recommended bounded `--once` command exits with code `3`, so its caller must retry. A recurring per-user Windows Scheduled Task is still required for automatic deletion when no later worker job runs. Registering that task changes machine state and remains a separate approval gate. A powered-off machine or disabled scheduler also prevents a strict wall-clock guarantee; this is namespace deletion, not forensic secure erasure.

`GET /api/anam/amy/readiness` is a content-free, bearer-authenticated readiness view that reuses the worker secret. It reports the effective session, recovery, and Hermes gates and proves that memory, tools, AgentMail, and global outbound actions remain unavailable. `POST /api/anam/hermes/worker` uses the same bearer authentication, accepts a maximum 32 KiB JSON body, and supports only `claim`, `begin`, `ack`, and `fail` operations. Claims require the exact `amy_anam_hermes_worker_v2` protocol value before any lease is created. Stop the worker during a server/worker update, deploy the server, then restart the matching worker; mixed versions fail before claim rather than bypassing `begin`.
...
The richer legacy intelligence fields such as top questions, pain points, and objections remain deferred. The input, provider, writer, and cleanup code boundaries are now hardened, but automatic Windows cleanup has not been registered and the richer provider-output schema has not been designed or approved. Neither gap should be bypassed by adding fields to the existing v1 output.
...
20. Do not change the production Amy persona mapping until the comparison passes and both the recovery schedule and local cleanup schedule are configured and observed.
