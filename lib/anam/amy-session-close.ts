...
const AMY_SOFT_CLOSE_INTENT = /\b(?:(?:let'?s|we can|we should)\s+(?:call it a day|wrap\s+(?:(?:it|this)\s+)?(?:up|here)|wrap\s+here)|(?:we(?:'re| are)|i(?:'m| am))\s+(?:all\s+set|good)(?:\s+for\s+(?:now|today))?|that(?:'s| is)\s+(?:all|everything)|nothing\s+else|wrap\s+(?:it|this)\s+here)\b/i;
