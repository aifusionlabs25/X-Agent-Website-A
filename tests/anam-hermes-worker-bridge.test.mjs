...
test('bridge request contracts reject extra fields, generated output, and invalid fail codes', () => {
    assert.deepEqual(normalizeAmyAnamHermesWorkerBridgeRequest({ operation: 'claim' }), {
        operation: 'claim',
    });
    assert.throws(
        () => normalizeAmyAnamHermesWorkerBridgeRequest({ operation: 'claim', sessionId: SESSION_ID }),
        /extra fields/,
    );
...
test('worker bridge is primary without local Redis credentials and never sends analysis content', async () => {
    ...
    try {
        ...
        assert.deepEqual(bridgeBodies.map(body => body.operation), ['claim', 'ack']);
        const ackBody = bridgeBodies[1];
